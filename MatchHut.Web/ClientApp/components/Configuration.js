        ...
        httpClient(`api/configurations?pageSize=${50}`, { method: 'GET' })
            ...
            .then(json => {
                json.forEach(c => {
                    ...
                    setValues(prevValues => ({ ...prevValues, [c.configCode]: c.configValue }));
                }, this);
            });
    }, []);

    const handleSubmit = () => httpClient('api/configurations', { method: 'PUT', body: JSON.stringify(values) })
        .then(response => {
        ...
        <>
            ...
            <div style={{ width: '100%' }}>
                <div style={styles.buttonContainer}>
                    <Button variant="contained" color="primary" startIcon={<SaveIcon />} style={styles.saveButton} onClick={handleSubmit}>Save</Button>
                    <ParamDownloadButton />
                </div>
            </div>
            <Card>
                <CardContent>
                    ...
                    <FormGroup row style={{ marginTop: 30 }}>
                        {values.vssIntegrated !== null &&
                            <FormControlLabel
                                control={<Switch name='vssIntegrated' checked={values.vssIntegrated} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Is Vss Enabled</Typography>}
                            />
                        }
                        {values.printOnlyRx !== null &&
                            <FormControlLabel
                                control={<Switch name='printOnlyRx' checked={values.printOnlyRx} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Print only rx</Typography>}
                            />
                        }
                    </FormGroup>
                    <FormGroup row style={{ marginTop: 30 }}>
                    ...
                    {values.followMarkup !== null &&
                        <FormGroup row style={{ marginTop: 30 }}>
                            <FormControlLabel
                                control={<Switch name='followMarkup' checked={values.followMarkup} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Follow Markup</Typography>}
                            />
                        </FormGroup>
                    }
                    ...
                    <FormGroup row>
                        {values.trancloudEnabled !== null &&
                            <FormControlLabel
                                control={<Switch name='trancloudEnabled' checked={values.trancloudEnabled} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Trancloud Enabled</Typography>}
                            />
                        }
                        {values.rxSignatureNeeded !== null &&
                            <FormControlLabel
                                control={<Switch name='rxSignatureNeeded' checked={values.rxSignatureNeeded} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Rx signature needed</Typography>}
                            />
                        }
                    </FormGroup>
                    <FormGroup row>
                        {values.loyaltyEnabled !== null &&
                            <FormControlLabel
                                control={<Switch name='loyaltyEnabled' checked={values.loyaltyEnabled} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Loyalty Enabled</Typography>}
                            />
                        }
                    </FormGroup>
                    <FormGroup row style={{ marginTop: 30 }}>
                    ...
                    <FormGroup row style={{ marginTop: 30 }}>
                        {values.creditCardLikeCash !== null &&
                            ...
                            <FormControlLabel
                                control={<Switch name='creditCardLikeCash' checked={values.creditCardLikeCash} onChange={handleSwitchChange} />}
                                label={<Typography style={{ fontSize: 12 }}>Credit Card like Cash</Typography>}
                            />
                        }
                    </FormGroup>
                </CardContent>

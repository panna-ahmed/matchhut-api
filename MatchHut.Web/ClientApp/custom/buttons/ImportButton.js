...
class ImportButton extends Component {
    ...
    checkMimeType = (event) => {
        ...
        const types = ['', 'csv', 'text/csv', 'application/vnd.ms-excel', 'application/csv', 'text/x-csv', 'application/x-csv', 'text/comma-separated-values', 'text/x-comma-separated-values'];
        // loop access array
    ...
    render() {
        const { state: { progressing, selectedFile, loaded } } = this;
        return (
            <>
                <Button
                    variant="contained"
                ...
                <Button variant="contained" color="default" disabled={!selectedFile} icon={
                    progressing ? (
                        <CircularProgress size={25} thickness={2} />
                    ) : null
                } onClick={this.onClickHandler} style={{ ...styles.importButton, marginLeft: 5}} startIcon={<CloudUploadIcon />}>
                    Import
                </Button>
